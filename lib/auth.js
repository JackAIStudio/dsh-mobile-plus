...
export class AuthManager {
  ...
  handleIssue = async (port, req, res) => {
    ...
    let needNode = Boolean(effectivePublic)
    if (effectivePublic) {
      try {
        const h = new URL(effectivePublic).hostname.toLowerCase()
        if (h.startsWith('dev.') || h.startsWith('dev-') || (isDesktop && h.startsWith('mac.'))) needNode = false
      } catch {}
    }
    const nodeSuffix = (needNode && effectivePublic !== '') ? `&node=${encodeURIComponent(nodeId)}` : ''
