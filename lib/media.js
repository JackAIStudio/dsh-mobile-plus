...
async function readAttachmentRaw(ctx, sessionId, attachmentId, filePath = '') {
  if (filePath) {
...
export async function handleAttachmentRequest(ctx, auth, req, res) {
  ...
  if (!attachmentId && !filePath) {
    ...
    res.end('Missing attachmentId or path')
    return
  }

  const cacheKey = `${attachmentId || filePath}:${isThumb ? `thumb-${targetWidth}` : 'raw'}`
  ...
  try {
    const raw = await readAttachmentRaw(ctx, sessionId, attachmentId, filePath)
    if (!raw || !raw.buffer) {
