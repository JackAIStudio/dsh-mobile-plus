...
import { json, readRawBody, svc } from './utils.js'
...
export async function sessionCwd(ctx, sessionId) {
  ...
  if (proxy) {
    ...
    try {
      ...
      if (sessionCtrl?.list) {
        const listed = await sessionCtrl.list({}, new AbortController().signal)
