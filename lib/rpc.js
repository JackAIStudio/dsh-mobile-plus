...
function formatWorkspaceView(ws) {
  if (!ws) return ws
  return {
  ...
  return async (method, payload, rpcId, signal) => {
      ...
      if (typeof wsCtrl?.feed?.baseline === "function") {
        ...
        return {
          ...
          result: {
            ...
            value: {
              items: base.items || [],
              archivedSessionIds: base.archivedSessionIds || [],
            },
          },
        }
      }
        ...
        if (wsRegistry?.create) {
          ...
          return {
            ...
            result: {
              ok: true,
              value: {
                workspace: formatWorkspaceView(entity),
                created: true,
              },
            },
          }
        }
        return { type: "server-response", rpcId, result: { ok: false, error: { code: "unavailable", message: "工作区服务不可用" } } }
      } catch (err) {
    ...
    if (method === 'session.list') {
      ...
      if (api?.sessions?.list) {
        ...
        try {
          const val = await sessionCtrl.list(payload || {})
          rawItems = val?.items || []
        } catch (err) {
          return { type: "server-response", rpcId, result: { ok: false, error: { code: err.code || "error", message: err.message } } }
        }
