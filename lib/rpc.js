...
import { isRecord, wrap, fetchLoopbackJson, svc } from './utils.js'
...
async function sessionsForWorkspace(ctx, items, workspaceId) {
  ...
  try {
    let wsList = []
    let apiProxy;
try { apiProxy = ctx.apiProxy } catch {}
if (apiProxy) {
      const listed = await apiProxy.workspace.list({ rpcId: `mp-ws-filter-${Date.now()}`, payload: {} })
      if (listed.result?.ok) wsList = listed.result.value.items || []
    } else if (typeof workspacesSvc?.list === "function") {
...
export function createDispatcher(ctx, pendingTracker) {
  return async (method, payload, rpcId, signal) => {
    ...
    const workspacesSvc = svc(ctx, "workspaces")
    ...
    if (method === 'workspace.list') {
      ...
      const items = (typeof workspacesSvc?.list === "function" ? ctx.workspaces.list() : []) || []
    ...
    if (method === 'workspace.create') {
      ...
      try {
        const val = await wsCtrl?.create?.(payload)
    ...
    if (method === 'session.create') {
      ...
      try {
        const val = await sessionCtrl?.create?.(payload)
        return { type: "server-response", rpcId, result: { ok: true, value: val } }
      } catch (err) {
        return { type: "server-response", rpcId, result: { ok: false, error: { code: err.code || "error", message: err.message } } }
      }
      ...
      if (api?.sessions?.history) {
        ...
        try {
          ...
          if (persistence?.inspect) {
            const inspected = await persistence.inspect(payload?.sessionId)
    ...
    if (method === 'session.prompt') {
      ...
      try {
        const val = await sessionCtrl?.prompt?.(next)
        return { type: "server-response", rpcId, result: { ok: true, value: val } }
      } catch (err) {
        return { type: "server-response", rpcId, result: { ok: false, error: { code: err.code || "error", message: err.message } } }
      }
    ...
    if (method === 'session.cancel') {
      ...
      try {
        const val = await sessionCtrl?.cancel?.(payload)
        return { type: "server-response", rpcId, result: { ok: true, value: val } }
      } catch (err) {
        return { type: "server-response", rpcId, result: { ok: false, error: { code: err.code || "error", message: err.message } } }
      }
    ...
    if (method === 'session.attachment') {
      ...
      try {
        const val = await sessionCtrl?.attachment?.(payload)
        return { type: "server-response", rpcId, result: { ok: true, value: val } }
      } catch (err) {
        return { type: "server-response", rpcId, result: { ok: false, error: { code: err.code || "error", message: err.message } } }
      }
    ...
    if (method === 'session.models') {
      ...
      try {
        const val = await sessionCtrl?.modelCatalog?.()
        return { type: "server-response", rpcId, result: { ok: true, value: val } }
      } catch (err) {
        return { type: "server-response", rpcId, result: { ok: false, error: { code: err.code || "error", message: err.message } } }
      }
    ...
    if (method === 'session.selectModel') {
      ...
      try {
        const val = await sessionCtrl?.selectModel?.(payload)
        return { type: "server-response", rpcId, result: { ok: true, value: val } }
      } catch (err) {
        return { type: "server-response", rpcId, result: { ok: false, error: { code: err.code || "error", message: err.message } } }
      }
    ...
    if (method === 'command.list') {
      ...
      if (!agent || !ctx.commands || typeof commandsSvc.list !== 'function') {
        return { type: 'server-response', rpcId, result: { ok: true, value: { items: [] } } }
      }
    ...
    if (method === 'command.execute') {
      ...
      const agent = sessionId && agentsSvc && typeof agentsSvc.get === 'function' ? agentsSvc.get(sessionId) : undefined
      if (!agent || !ctx.commands || typeof commandsSvc.execute !== 'function') {
        return {
      ...
      const execution = await commandsSvc.execute(agent, line, [], signal)
