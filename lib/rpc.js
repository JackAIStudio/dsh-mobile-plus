...
import { foldHistoryForMobile, paginateSessions, formatWorkspaceView, sessionsForWorkspace, attachContextPressure } from './history.js'
  ...
  return async (method, payload, rpcId, signal) => {
    ...
    if (method === 'session.history') {
      ...
      if (full.result?.ok) {
        ...
        attachContextPressure(full.result.value, folded, ctx, payload?.sessionId)
      }
