 ...
 * Regular projects are displayed in the main list,
 * while day-based daily workspaces are grouped into a foldable section.
 */
...
export function visibleWorkspaces() {
  ...
  const filtered = q ? filterWorkspaces(all, q) : all
  return sortWorkspaces(filtered, state.sortMode, runtime)
}
...
export function renderWorkspaces() {
  ...
  const quickCards = el('div', { class: 'mobile-quick-grid' }, [
    el('button', {
      ...
      onclick: () => {
        if (todayMatch) void openWorkspace(todayMatch)
        else void openToday()
      },
    }, [
  ...
  const renderFoldSection = (daily) => {
    const sortedDaily = sortDailyWorkspaces(daily)
    const foldHeader = el('button', {
      type: 'button',
      class: `ws-fold-header${wsDailyExpanded ? ' is-expanded' : ''}`,
      onclick: () => {
        wsDailyExpanded = !wsDailyExpanded
    ...
    if (wsDailyExpanded) {
      kids.push(el('div', { class: 'ws-fold-content' }, [
        el('p', { class: 'ws-fold-hint' }, ['💡 今日工作区可直接点击上方「今日工作区」卡片快速直达']),
        ...sortedDaily.map((ws) => workspaceRow(ws)),
      ]))
    }
    return el('li', { class: 'ws-fold-li' }, [
      el('div', { class: 'ws-fold-card' }, kids)
    ])
  }
  ...
  const refreshWorkspaceList = () => {
    ...
    if (!q) {
      const { projects, daily } = partitionWorkspaces(all)
      const sortedProjects = sortWorkspaces(projects, state.sortMode, runtime)
      const rows = sortedProjects.map(workspaceRow)
      if (daily.length > 0) {
        rows.push(renderFoldSection(daily))
      }
      list.replaceChildren(...rows)
      if (all.length === 0) {
        empty.textContent = '还没有工作区'
        empty.hidden = false
      } else {
        empty.hidden = true
      }
    ...
    const filtered = filterWorkspaces(all, q)
    const sorted = sortWorkspaces(filtered, state.sortMode, runtime)
    list.replaceChildren(...sorted.map(workspaceRow))
    if (sorted.length === 0) {
      empty.textContent = `没有匹配「${q}」的工作区`
      empty.hidden = false
    } else {
      empty.hidden = true
    }
