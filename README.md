...
4. 进入「工作区」，选一个工作区，再选会话（或 `+ 新建会话`）。若电脑装着 `dsh-today`，工作区页眉右侧会多一颗日历 logo，点一下打开当天的 `days/YYYY-MM-DD`。当前工作区/会话会写进地址栏（`#/ws/…/s/…`）并记住上次位置：刷新浏览器或从主屏幕重新打开 PWA，会回到刚才的会话，而不是工作区列表；会话或工作区已经不在了，则退到还能打开的那一层
